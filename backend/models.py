...
class User(db.Model):
    ...
    password_hash = Column(String(512), nullable=True)  # Made nullable for social login
...
class School(db.Model):
    ...
    quizzes = relationship("Quiz", back_populates="school")  # Add this line
    attendances = relationship("Attendance", back_populates="school", cascade="all, delete-orphan")
    chats = relationship("Chat", back_populates="school", cascade="all, delete-orphan")
...
class Attendance(db.Model):
    ...
    id = Column(Integer, primary_key=True)
    class_id = Column(Integer, ForeignKey("classes.id"), nullable=False)
    school_id = Column(Integer, ForeignKey("schools.id"), nullable=False)  # ✅ Added school_id
    student_id = Column(Integer, ForeignKey("users.id"), nullable=False)
...
class Chat(db.Model):
    ...
    school = relationship("School", back_populates="chats")
...
class Quiz(db.Model):
    ...
    id = Column(Integer, primary_key=True)
    class_id = Column(Integer, ForeignKey("classes.id"), nullable=False)
    school_id = Column(Integer, ForeignKey("schools.id"), nullable=False)  # Add school_id
    quiz_title = Column(String(150), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)

    # Relationships
    class_ = relationship("Class", back_populates="quizzes")
    school = relationship("School", back_populates="quizzes")  # Add relationship to School
